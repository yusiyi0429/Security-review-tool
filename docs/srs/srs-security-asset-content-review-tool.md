...
| 交付形态 | `win-x64` 自包含便携目录 ZIP，无安装器、服务和自动更新 |
...
便携指解压即用、无管理员权限、无安装器、无服务、程序目录可整体替换；不表示完全不写入用户配置。运行数据放在：
...
| SRS ID | 软件需求 | 上游 | 验证 |
| --- | --- | --- | --- |
| SRS-F-001 | 提供简体中文 WPF `win-x64` 自包含便携目录；无需管理员权限，不安装服务或自动更新器，并在 5 秒目标内显示可交互主窗体。 | REQ-001 / AC-001 | VT-001, VT-002 |
...
| VT | 主要断言 |
| --- | --- |
| VT-001 | clean supported Windows VM 免管理员启动、中文 UI、无服务/安装痕迹 |
| VT-002 | 冷启动、空闲内存和自包含便携目录验证 |
